...
async def get_user_info(username: str):
    ...
    user_and_his_videos = await services.get_user_and_his_video_by_username(username=username)

    if not user_and_his_videos:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found!")

    subscribers = await services.get_user_subscribers(username)

    return user_and_his_videos.dict() | {"subscribers": subscribers}


@user_router.post("/sign-up", response_model=UserSchema, status_code=status.HTTP_201_CREATED)
async def create_user(form_data: UserCreate, background_task: BackgroundTasks):
    ...
    user = await services.get_user_by_email(email=form_data.email)
    if user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User with that email already registered!")

    return await services.create_user(user=form_data, background_task=background_task)
...
async def update_user(update_data: UserUpdate, current_user: Users = Depends(get_current_user)):
    ...
    user = await services.get_user_by_username(username=update_data.username)
    if user and user != current_user:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="User with that username already registered!"
        )

    user = await services.get_user_by_email(email=update_data.email)
    if user and user != current_user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User with that email already registered!")
